...
int buscaBinaria(int *v, int *p, int l, int r, int n) {
    if(l > r) return -1;
    int meio = (l + r) / 2;
    if(v[meio] == n) return p[meio];
    else if(v[meio] > n) r = meio - 1;
    else l = meio + 1;

    buscaBinaria(v, p, l, r, n);
}

void selectionsort(int *v, int *p, int l, int r) {
    for(int i = 0; i < r; i++){
        int min = i;
        int j = i + 1;
        while(j <= r){
